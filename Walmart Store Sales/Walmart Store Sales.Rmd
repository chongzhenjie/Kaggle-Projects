...
```{r}
train_df <- read_csv("../input/walmart-recruiting-store-sales-forecasting/train.csv.zip")
glimpse(train_df)

test_df <- read_csv("../input/walmart-recruiting-store-sales-forecasting/test.csv.zip")
glimpse(test_df)

stores <- read_csv("../input/walmart-recruiting-store-sales-forecasting/stores.csv")
glimpse(stores)

features <- read_csv("../input/walmart-recruiting-store-sales-forecasting/features.csv.zip")
glimpse(features)
```
...
We can also create an average model by taking the average across the forecasts of the above models. This helps to average out the errors.
...
The `WMAE` for the average of the weaker models perform significantly better. In fact, taking the average across all individual models results in an even better score.
...
A thing to note is that for those `storeDept` without any historical observations, we let the forecasts take value 0.
